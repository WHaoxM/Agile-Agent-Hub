...
GROUP_CHAT_SUMMARY_PROMPT = """你是一个专业的群聊总结助手。请根据给定的群聊历史生成一份完整的总结，并提取所有待办任务。

## 输入格式
输入是一段长的中文群聊历史文本，按时间顺序排列，可能包含多个主题。

## 输出格式
请返回一个 JSON 对象，包含以下字段：
- summary: 字符串，完整的群聊总结（必填，100-500字）
- tasks: 数组，所有提取到的待办任务（必填，每个任务的格式如下）
  - task: 字符串，任务描述（必填）
  - assignee: 字符串或 null，负责人姓名（可选）
  - deadline: 字符串或 null，截止时间（可选，格式如 "2026-04-14 15:00"）
  - raw_text: 字符串，包含所有相关的原始文本片段（必填）
...
输出：
{{
  "summary": "本次群聊主要讨论了下周的项目演示安排。确定了演示时间为下周三下午2点，地点在会议室A。李四将负责准备演示文稿，王五负责在周五前更新演示所需的最新数据。",
  "tasks": [
    {{
      "task": "准备项目演示文稿",
      "assignee": "李四",
...
def build_group_chat_summary_prompt(chat_text: str, current_date: str) -> str:
    """构建群聊总结和任务提取的提示词模板。

    Args:
        chat_text: 群聊历史文本，不能为空字符串或仅包含空白字符
        current_date: 当前日期字符串，格式应为 "YYYY-MM-DD"
    ...
    return GROUP_CHAT_SUMMARY_PROMPT.format(
        chat_text=chat_text,
        current_date=current_date
    )
